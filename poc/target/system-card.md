...
> Describes the **original** stand only — no red-team instrumentation, no LangFuse.
> The architecture diagram is in `arch.mmd`. Machine-driving parameters are in `target.yaml`.
