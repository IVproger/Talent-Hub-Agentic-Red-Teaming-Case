...
ядро обслуживает CLI и локальный UI, поэтому гейты авторизации и режима без
записи действуют одинаково на обоих входах. Сценарии варьируют локальные
модели; LLM предлагает payload и переформулирует отчёт, но не выносит
security-вердикт. На выходе — технический отчёт, бизнес-отчёт, база знаний и
regression-тест.

Проверено следующее. Собран сквозной workflow: профиль → карта поверхности →
генерация → кампания → evidence → два отчёта → replay; инженерная база — 493
теста, зелёный набор. На учебном стенде доказаны 4 из 4 атак класса Broken
Access Control: агент от имени одного клиента фактически вызвал инструмент с
...
### 3.8. Что команда написала сама

Не список библиотек, а вклад: единый campaign runner для CLI и UI · профильная
архитектура без target-specific кода в ядре · шесть evidence-провайдеров и
bundle с нормализацией · детерминированные предикаты и градация вердикта ·
composer «шаблон × профиль» и генератор payload с дедупликацией и контекстом
прошлых кампаний · lifecycle находки от `confirmed` до `closed/reopened` ·
регрессия `export → run --from → compare` · два отчёта · сквозная W3C-трасса ·
...
| Инженерная база: **493 теста, набор зелёный** (последний зафиксированный snapshot 05.09.2026) | `[ФАКТ]` |
...
| Объём: 189 коммитов за 5 дней, ≈ 9 000 строк движка и ≈ 7 200 строк тестов | `[ФАКТ]` |
...
- **Продуктовая экономика не измерена.** Все TO-BE значения — модель, метрики
  снимаем на пилоте.
- **Число тестов 493** — последний зафиксированный snapshot; `CLAUDE.md`
  содержит более раннее значение 441, актуальным считается STATUS.md.
