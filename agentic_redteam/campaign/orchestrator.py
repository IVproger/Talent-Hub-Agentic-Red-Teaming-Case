...
def _run_campaign(scenarios, deps: RunnerDeps, storage, run_id: str,
    ...
    def checkpoint(current_status):
        ...
        findings["generation"] = record.get("generation", {})
        generation_totals = findings["generation"].get("totals", {})
        findings["diversity"]["new_payloads"] = generation_totals.get("new", 0)
        findings["diversity"]["repeat_payloads"] = generation_totals.get("repeat", 0)
