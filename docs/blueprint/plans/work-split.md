...
>
> **Область действия:** задачи Ядра, взятые 5 сентября 2026. Документ не
> описывает роли команды в целом. Вклад в ресерч, идею и продуктовые
> артефакты 2-4 сентября распределялся иначе — источник по нему `artifacts/`
> и история git (файлы переносились, нужен `git log --follow`).
...
| Тамерлан | `oushtt` | профиль, адаптер, evidence-провайдеры, llm, стенд |

Исполнителей двое. Продуктовые артефакты (шаблоны E3, бизнес-отчёт E9) на
отдельного человека не выделяются: шаблоны уже реализованы в `templates/` и
`generation/`, бизнес-отчёт E9 отложен.
...
Фаза 0 (оба, ПЕРВЫМ делом, параллельно):
   dseredkin: 0.0 0.1 0.2        oushtt: 1.1 2.1 3.1
        │                              │
Фаза 1 (параллельно, независимо):
   dseredkin: S1 (0.3–0.7)       oushtt: S2 (1.2–1.3) · S3 (2.2–2.4) · S4 (3.2–3.7)
        └──────────────┬───────────────┘
Точка сборки — S5 runner (dseredkin, 4.1–4.4): нужны и ядро, и профиль/адаптер/evidence.
        │
Фаза 3 (параллельно):
   dseredkin: S6 (5.1–5.5) · 6.2 · 6.3      oushtt: 6.1 · 6.4
```

**Главное:**
- **Фаза 0 — первой** у обоих: замораживает контракты (`facts`/`verdict` у dseredkin, `TargetProfile`/`TargetAdapter`/`EvidenceProvider` у oushtt). Пока не готова — Фаза 1 не начинается.
- **S5 runner (dseredkin)** ждёт, пока oushtt закроет S2/S3/S4 — это единственная жёсткая межчеловеческая зависимость.
...
| oushtt | 16 | граница цели + данные (глубже по интеграциям) |
