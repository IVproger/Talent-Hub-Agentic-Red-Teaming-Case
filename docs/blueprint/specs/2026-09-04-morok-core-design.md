...
результат разбора артефактов; при онбординге их подтверждает LLM-судья, а не человек (§4.3). А
...
Пользователь даёт документы, ingest собирает из них профиль, а
семантические привязки подтверждает LLM-судья при онбординге, без человека (§1.1). Новый адаптер нужен лишь под
...
## 12. CLI: поверхность команд

Инвокация: `python -m agentic_redteam <cmd>` (условно `morok <cmd>`). CLI —
основной интерфейс (кейсодатель: «нужен скрипт/CLI, UI не цель»); Streamlit —
только демо. Команды — обёртки над runner (§6) и профилем (§1).

### Дерево команд

| Команда | Что делает | Слой | US |
|---|---|---|---|
| `profile init` | черновик профиля из артефактов | Ядро+ingest | US-01 |
| `profile check` | read-only проверка подключения → `surface.json` | Ядро | US-06 |
| `profile verify` | проба видимости (меняет состояние) | Ядро | US-06 |
| `profile coverage` | что проверяемо / гейт покрытия | Ядро/E3 | US-04 |
| `profile list` | список профилей | Ядро | US-02 |
| `profile show` | карта поверхности профиля | Ядро | US-05/07 |
| `profile diff` | различия двух версий | Ядро | US-02 |
| `run` | прогнать кампанию по профилю | Ядро | US-15/16/18 |
| `report` | пересобрать отчёт из `runs/<id>` | Ядро | US-26 |
| `doctor` | preflight окружения (→ `check`) | Ядро | — |
| `stand sync` · `serve` | встроенный стенд · Streamlit-демо | есть | — |
| `kb` · `regress` · `report --business` | база знаний · регрессия · бизнес-отчёт | эпики | E6/E8/E9 |

### Решения

- **Адресация профиля:** `--profile name@version` (реестр) или `--profile path.yaml` (прямой путь).
- **Состав кампании — флагами:** `--scenario … --trials N --mode vulnerable,protected`; `--scenario all` = весь применимый каталог. Кампания авто-сохраняется в `runs/<id>/campaign.json`; повтор — `run --from runs/<id>`. Отдельного файла кампании нет.
- **Предпросмотр — `--dry-run`:** печатает план; для генерируемых сценариев показывает сами payload'ы до отправки (US-16 AC2); после подтверждения выполняется ровно показанное.
- **`profile init`:** механика извлекает **структуру** (tools/args/entrypoint) из OpenAPI детерминированно; analyst-LLM предлагает **семантические привязки** (`principal_from`, `scope`, `sensitive`, `isolation`) из прозы system-card, а judge-LLM их валидирует (`accept`/`reject` + обоснование + `confidence`). Принятые применяются в профиль автоматически с меткой `provenance: llm-judged`; отклонённые сохраняются в `ingest.judgement` и **не** применяются. Человек из цепочки убран осознанно: такие привязки стоят на суждении LLM, а не на наблюдении, поэтому профиль от `init` помечен `llm-judged` и не эквивалентен наблюдаемому `state-confirmed`. Наблюдаемое подтверждение — отдельный `profile verify` (проба состоянием); `init` его не выполняет. `--offline` — без LLM: привязки помечаются `TODO` (слабые эвристики по именам). Структура парсится, семантика — инференс, парсером не выводится.
- **Вывод:** человекочитаемый + `--json`. Exit codes: `0` успех, `2` конфиг, `3` preflight target, `4` LLM provider, `5` pipeline.
