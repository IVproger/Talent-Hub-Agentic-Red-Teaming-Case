...
Этот раздел имеет приоритет над историческими аудитами ниже.

- **3 — W3C/Langfuse tracing: закрыто кодом, тестами и live-run.** Прогон
  `20260905-192427-e7f221` создал trace
  `8ff45f3b1396f50fbe4e3b16c1603079` с 19 наблюдениями. Проверена реальная
  родительская цепочка `redteam.run → campaign.attempt → stand.chat →
  stand.react.loop → stand.tool.*`, а не только наличие `traceparent` в unit-
  тесте. Полный протокол: [live Langfuse validation](live-langfuse-validation-2026-09-05.md).
- **5 — surface map и генерация с историей: закрыто.** Карта уже объединяет
  каналы, entrypoint, sensitive tools, память, интеграции, evidence, связи и
  доказуемое ASI-покрытие. Генератор теперь действительно использует историю
  KB и документы цели, помечает новые варианты и намеренные replay-повторы
  `new`/`repeat` и сохраняет точный
  контекст и классификацию в preview, saved plan, `campaign.json`,
  `findings.json` и отчёте.
- **8 — lifecycle находок и документация: закрыто.** Replay автоматически
  переводит только исполненные state-доказанные записи `fixed → retested →
  closed|reopened`, сохраняет историю и не переписывает ручные статусы.
...
## Интеграция веток — текущий `main`

Rebase `478305f` поверх `08cd40f` сведён без потери функций обеих линий:

- единый `execute_campaign` сохраняет authorization/read-only гейты и
  одновременно profile snapshot, preflight, telemetry/W3C, replay и safe stop;
- CLI содержит baseline/smoke/save-plan/profile ingest/surface вместе с
  regress и knowledge lifecycle;
- генератор получает одновременно документы цели и контекст прошлых кампаний;
- старые regression/smoke campaign-артефакты нормализуются при replay;
- YAML-даты effective config сериализуются в стабильный ISO-формат.

Проверка на момент интеграции: **472 теста, OK**. E7/E9 и расширенная карта E2
реализованы последующими атомарными коммитами. Упоминание непроверенной
сквозной телеметрии здесь историческое: актуальный live-результат приведён выше.
...
### E2 — расширенная карта поверхности

- `surface/map.py` теперь формирует единый threat-model словарь с точкой входа,
  каналами доставки, sensitive-приоритетом инструментов, памятью, внешними
  интеграциями и явными связями `channel → entrypoint → tool/memory` и
  `integration → evidence/memory`.
- Каналы объединяют декларацию профиля и delivery применимых шаблонов; поэтому
  неподдержанный шаблон не расширяет scope молча. Не проверяемый отдельным
  probe канал остаётся «заявлен, но не подтверждён».
- `coverage` переиспользует существующий composer-гейт и считает уникальные
  state-доказуемые ASI-пункты из 10, не выдавая число шаблонов за полноту
  стандарта. Детальные строки и причины исключения остаются в JSON.
...
из обеих зон.
## Аудит текущего checkout

Подробная сверка исходных пунктов 1–4 с кодом, тестами и live-артефактами:
[реализация 1–4](implementation-1-4-2026-09-05.md#аудит-фактического-состояния--2026-09-05).

Кратко: надёжность прерывания, ASR, optional-memory, `per_step`, UI и удаление
старого pipeline закрыты в `main`. Исполняемый `run --from`, полноценный
telemetry-root/W3C propagation и расширенный поток profile/surface/baseline
работают в локальном checkout, но ещё не закоммичены. Главный незакрытый гейт —
живой `proven` для цепочки «отравление памяти → другая роль/сессия → чужой tool
call» и повтор того же сценария на второй реальной цели.

Прежняя запись о том, что многошаговый сценарий не запускался, устарела:
он выполнен 3× в каждом режиме, но остался `not_proven`; подробности добавлены
в [live-validation](live-validation-2026-09-05.md#обновление-многошаговая-цепочка-после-исправления-атрибуции).
...
11. ~~**Сквозная трасса**~~ — **сделано позднее**: адаптер пробрасывает W3C
    `traceparent`, стенд продолжает контекст, живой Langfuse trace содержит
    ReAct-цикл и tool calls цели. См. актуальный срез в начале документа.
