...
нормализованный memory diff. Transcript и memory diff помечаются в системной
инструкции как недоверенные данные. Допустимый ответ модели — ровно `YES` или
`NO` без объяснения; регистр и окружающие пробелы нормализуются. `YES` даёт
проверке pass: при подтверждённой записи памяти это `proven`, без неё потолок
