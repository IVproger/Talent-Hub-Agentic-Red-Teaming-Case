...
про `cus`, ни про Mongo, ни про наш стенд — так, чтобы один и тот же
встроенный сценарий шёл и на `genai-invest-stand`, и на DVAA без правки
кода движка.
...
- **Telemetry fail-open, но evidence — нет.** Langfuse как наблюдаемость
  *нашего прогона* не влияет на вердикт и падает молча (warning). OTel/лог
  цели как *источник evidence* обязателен, когда сценарий его требует; его
  отказ → `error`. Это разные объекты в профиле.
...
- добавляется `reset_policy: per_scenario | per_step | none` (политика
  сброса цели; ортогональна режиму и петле генерации);
...
tool calls из docker-log/файла/endpoint; `http_canary` — внешний
callback; `json_file` — файловое хранилище; `state_reset` — reset.
...
веткой: генератор (эпик E4) производит обычный `Scenario`, runner его не
отличает от встроенного.

Порядок циклов: **по режимам → по сценариям → по попыткам**. При
`modes.scope == per_deployment` попытки группируются по режиму (переключение
= передеплой). Reset цели — по `scenario.reset_policy`, не по режиму и не
по петле. `attribution: serialized` → runner требует эксклюзивный доступ,
и это допущение пишется в условия воспроизведения отчёта.
...
(для реплея адаптивных петель — эпик E4).
...
детерминирован, LLM только в тексте payload и выборе шага петли ·
адаптивная петля: сначала режим «поиск», диалог — расширением
...
|---|---|---|
| E2 карта поверхности | surface-map | profile.surface + calibrate |
| E3/E4 шаблоны + composer + генерация-петля | attack-generation | Scenario + assertions |
