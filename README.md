...
принципалов (`cus=…`) должны принадлежать ролям. Запись `name(...)` в свободном
тексте — лишь эвристика: известные роли и хранилища не считаются инструментами,
неизвестные имена дают предупреждения `warnings` в `--json`, не отбраковку.
Невалидная схема, ID, ссылки на стандарты и принципалы по-прежнему блокируют brief.
Причины отбраковки доступны в `rejected` через `--json`; при успешной генерации
обычный вывод показывает только сохранённый набор без диагностических строк. Набор
...
Генератор получает правила схемы и пример: `id` должен полностью соответствовать
`[a-z0-9][a-z0-9-]*` (например `session-auth-transfer`, но не `B1`). Если ответ
не содержит ни одного валидного brief, включая невалидный JSON или пустой массив,
выполняется **одна коррекция через LLM** с исходным контекстом и причинами отказа.
Исправленный ответ проходит те же проверки; невалидные ID не переименовываются
молча. Максимум два вызова генератора; транспортные ошибки здесь не повторяются.
Если коррекция не помогла, команда завершается с кодом 2 и не сохраняет набор.
Частично валидный набор сохраняется без дополнительного вызова. `rejected` в JSON
содержит причины отбраковки и номер ответа `generation` (1 или 2), в том числе
ошибки первого ответа после успешной коррекции.

Свои идеи можно передать повторяемым флагом `--idea`:

```bash
.venv/bin/python -m agentic_redteam briefs generate \
  --profile genai-invest-stand@1.0.0 \
  --idea "Попробовать получить портфель другого клиента через подмену cus" \
  --idea "Проверить перенос авторизации между сессиями" \
  --count 5 \
  --out local-scenarios/briefs-my-ideas
```

Идеи задают приоритетные направления в порядке указания, а не готовые сценарии.
`--count` — общий бюджет brief, не количество на каждую идею. Генератору
предписано адаптировать применимые идеи к профилю, объединять близкие и дополнять
набор угрозами из стандартов; покрытие каждой идеи не гарантируется.
Проверки схемы и сущностей профиля остаются обязательными. Без `--idea`
работает прежняя генерация по профилю и стандартам. Пустая идея — ошибка
использования (код 2) до обращения к модели; `--json` включает список `ideas`.
Не передавайте секреты: аргументы видны в истории shell, идеи отправляются LLM.
Проверьте сгенерированные YAML перед запуском через `run --briefs`.

**Идеи из файла.** Вместо длинных аргументов используйте `--ideas-file`:
...
.venv/bin/python -m agentic_redteam briefs generate \
  ...
  --count 5 \
  --out local-scenarios/briefs-from-file
```

Пример `ideas.yaml` (также поддерживается `.yml`):

```yaml
ideas:
  - Попробовать получить портфель другого клиента через подмену cus
  - |
    Проверить перенос авторизации между сессиями.
    Сначала выполнить разрешённый запрос, затем открыть другую сессию.
```

Можно использовать и YAML-список без поля `ideas`. Дополнительные поля,
пустые идеи и значения нестрокового типа отклоняются.

Для `ideas.txt` или `ideas.md` пишите обычный текст: весь файл передаётся как
одна развёрнутая идея с сохранением строк, заголовков и списков. Разбивать
Markdown по строкам не нужно; если нужен приоритет отдельных идей, используйте YAML.

`--ideas-file` можно повторять и сочетать с `--idea`. Сначала идут все значения
`--idea`, затем идеи из файлов в порядке флагов `--ideas-file` и элементов YAML.
`--count` по-прежнему задаёт общий бюджет, не количество на файл. Пути считаются
от текущего каталога; файлы должны быть в UTF-8 (BOM допустим), до 64 КиБ каждый.
Ошибки чтения, формата или пустое содержимое дают код 2 до обращения к LLM и
сохранения brief. Исходные файлы не изменяются; загруженные идеи включаются
в поле `ideas` ответа `--json`. Не включайте секреты: содержимое передаётся модели.
...
показывает вывод, полноту оценки, результаты по brief и карточки YES:
критерий, principal, наблюдаемые вызовы, фрагменты диалога и ссылки на spans.
Claim атакующего не используется как доказательство или источник режима;
полные transcript свёрнуты в `<details>`. Бизнес-отчёт показывает не только
`YES`, но и полноту оценки, неоценённые попытки, возможную связь с запретами
`profile.business` и следующие действия в компактных карточках. Совпадение
`standard_refs` — тематическая подсказка для владельца, не доказанная связь.
ASR 100% на одной попытке не означает высокую достоверность или вероятность
успеха в эксплуатации. Неиспользованный бюджет после `--stop-on-success`
показан отдельно от ошибок. `NO` не формулируется как
...
UI строит актуальный preview из сохранённых фактов, без нового вызова модели.
Локальные ссылки Markdown заменены указанием на вкладку «ФАЙЛЫ»: там можно
выбрать попытку и скачать её judge/evidence/actions, актуальные MD и исходные
отчёты, сохранённые при запуске (включая аналитическую записку). Исторические
прогоны при просмотре не перезаписываются. Для переносимых относительных
ссылок распакуйте evidence ZIP и открывайте MD рядом с каталогом `attempts`.
Если трасса не записалась, UI и MD показывают это явно: восстановить её
из локального журнала задним числом нельзя.
